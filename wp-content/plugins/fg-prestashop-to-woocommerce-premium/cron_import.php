...
if ( !defined('ABSPATH') ) {
	...
	$cron = new FG_PrestaShop_to_WooCommerce_Cron();
	$cron->run();
}
 ...
 */
class FG_PrestaShop_to_WooCommerce_Cron {
	...
	private function do_action($action) {
		...
		$fgp2wcp->display_admin_notice("=== START $action $time_start ===");
		
		echo $fgp2wcp->dispatch($action);
		
		$time_end = date('Y-m-d H:i:s');
		$fgp2wcp->display_admin_notice("=== END $action $time_end ===\n");
	}
